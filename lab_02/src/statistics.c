...
void show_ticks_statistics(table_countries_t *table_countries)
{
    ...
    double diff_1 = - 100.0 * (1.0 - (double) ticks[T_K_SORT_TIME_I][QSORT_I] /
    (double) ticks[T_C_SORT_TIME_I][QSORT_I]);

    double diff_2 = - 100.0 * (1.0 - (double) ticks[T_K_SORT_TIME_I][SEL_SORT_I] /
    (double) ticks[T_C_SORT_TIME_I][SEL_SORT_I]);

    printf("Сортировка таблицы стран │ %16zu │ %16zu\n",
    ticks[T_C_SORT_TIME_I][QSORT_I], ticks[T_C_SORT_TIME_I][SEL_SORT_I]);

    printf("Создание таблицы ключей  │ %16zu │ %16zu\n",
    ticks[T_K_GEN_TIME_I][QSORT_I], ticks[T_K_GEN_TIME_I][SEL_SORT_I]);

    printf("Сортировка таблицы ключей│ (%4.2lf%%) %6zu │ (%4.2lf%%) %6zu \n",
    diff_1, ticks[T_K_SORT_TIME_I][QSORT_I],
    diff_2, ticks[T_K_SORT_TIME_I][SEL_SORT_I]);
